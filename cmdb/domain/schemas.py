"""Pydantic output models shared by the MCP tools and the read-only web API.

These serialize SQLAlchemy ORM objects (``from_attributes=True``) into compact,
JSON-friendly shapes. Every consumer must build these *inside* an open session
so lazy-loaded relationships (tags, containers, ...) resolve before the session
closes — see ``cmdb/mcp/server.py`` and ``cmdb/web/routes/api.py``.

``raw_facts`` (the large per-host JSON blob) is deliberately excluded from the
default host models to keep output small; expose it via a dedicated endpoint if
ever needed.
"""
...
class ContainerWithHostOut(ContainerOut):
    """Flat container row for fleet-wide lists (adds the owning host)."""

    host: str | None = None

    @field_validator("host", mode="before")
    @classmethod
    def _hostname(cls, v: Any) -> str | None:
        # from_attributes hands us the ORM Host relationship; keep its name.
        return getattr(v, "hostname", v)
...
# --- Serializer helpers shared by MCP and the web API ------------------------


def running_on(row: dict) -> list[str]:
    """Human-readable placements from an image_overview/image_status row."""
    docker = [
...
def image_summary_out(session, image, row: dict | None = None) -> ImageSummaryOut:
    """Build an ImageSummaryOut from an Image (+ optional precomputed row)."""
